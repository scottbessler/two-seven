...
export function CardSettings({ settings: providedSettings, setSettings: providedSetSettings, interactive = false, concealable = false, trigger = true, children = null } = {}) {
  ...
  const dialog = html`<dialog id="card-config" class="card-config-dialog">
      <form method="dialog">
        ...
        ${concealable && html`<label class="card-option-toggle"><input name="confirm-fold" type="checkbox" checked=${settings.confirmFold} onChange=${toggle("confirmFold", CARD_SETTING_KEYS.confirmFold)} /><span><b>Hold to fold</b><small>Require a 1-second hold to fold</small></span></label>`}
        ${concealable && html`<label class="card-option-toggle"><input name="confirm-all-in" type="checkbox" checked=${settings.confirmAllIn} onChange=${toggle("confirmAllIn", CARD_SETTING_KEYS.confirmAllIn)} /><span><b>Hold for all-in</b><small>Require a 1-second hold before committing every chip</small></span></label>`}
        ${children}
      </form>
    </dialog>`;
  return trigger
