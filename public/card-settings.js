...
export function applyCardSettings({ cardSize, rankSize, rankWeight: rankBoldness }) {
  ...
  root.style.setProperty("--viewer-corner", `${0.52 * scale}rem`);
  root.style.setProperty("--viewer-pip", `${0.68 * scale * contentScale}rem`);
  root.style.setProperty("--viewer-art", `${1.7 * scale * contentScale}rem`);
...
export function CardSettings({ settings: providedSettings, setSettings: providedSetSettings, interactive = false } = {}) {
