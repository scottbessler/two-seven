...
test("anchors narrow metrics and keeps the result action bar steady", async ({ page }) => {
  ...
  const live = await page.locator(".table-shell").evaluate((shell) => {
    ...
    const seats = [...stageNode.querySelectorAll(".other-seats .seat")].map((seat) => seat.getBoundingClientRect());
    return {
...
test("offers one state-aware table lifecycle command", async ({ page }) => {
  ...
  // V62: the stakes decide who the house will sit. A $20,000 seat is still
  // below the no-fish rung, so every kind is on offer.
  await expect(page.locator(".seat-bot button")).toHaveText(["Seat fish", "Seat rock", "Seat grinder", "Seat shark"]);
  // A $100,000 seat has dropped the fish but keeps the rest; from $500,000 up
  // a shark is the only thing on offer.
  await mountTable(page, { ...unseated, bank_balance: 2_500_000, buy_in: 10_000_000 });
  await expect(page.locator(".seat-bot button")).toHaveText(["Seat rock", "Seat grinder", "Seat shark"]);
  await mountTable(page, { ...unseated, bank_balance: 2_500_000, buy_in: 50_000_000 });
  ...
  for (const device of [IPHONE_PORTRAIT, IPHONE_SE_PORTRAIT, IPHONE_MAX_PORTRAIT]) {
    ...
    const layout = await page.locator(".table-shell").evaluate((shell) => {
      ...
      return {
        ...
        boardSpansStage: Math.abs(sharedCards.width - stageBox.width) <= 1,
...
test("runs an all-in board out one street at a time", async ({ page }) => {
  ...
  const revealLayout = await page.locator(".table-stage").evaluate((stage) => {
    ...
    return {
      ...
      regrid: stage.querySelectorAll(".other-seats .seat").length === 5,
      oddsBelowBoard: odds.top >= boardBox.bottom,
      oddsWithinBoard: odds.left >= boardBox.left - 1 && odds.right <= boardBox.right + 1,
...
test("a five-handed phone table fills every cell and spends the surplus on the board", async ({ page }) => {
  ...
  // take the sixth cell, and the board answers to the whole stage width.
  await useDevice(page, IPHONE_PORTRAIT);
  await mountTable(page, tableState);
  await expect(page.locator(".other-seats")).toHaveAttribute("data-seat-total", "5");

  const stage = await page.locator(".table-stage").evaluate((element) => {
    ...
    return {
      ...
      seats: [...element.querySelectorAll(".other-seats .seat")].map((seat) => {
        const rect = seat.getBoundingClientRect();
        return { x: Math.round(rect.x), y: Math.round(rect.y) };
      }),
      metrics: box(".table-metrics"),
      board: box(".board"),
      card: element.querySelector(".board .playing-card").getBoundingClientRect().width,
    };
  });

  expect(stage.columns, "five opponents regrid to three columns").toBe(3);
  // Two rows of three cells, and the sixth is the pot: nothing is stranded.
  expect(new Set(stage.seats.map((seat) => seat.y)).size).toBe(2);
  expect(stage.metrics.x, "the pot chips take the cell the fifth seat leaves").toBeGreaterThan(stage.seats[4].x);
  expect(stage.metrics.right).toBe(stage.board.right);
  // The board no longer sits between two fixed rails.
